...
function parse_rst(){
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		until [[ ! -z "${lines["$parent_line_number,type"]}" ]] &&
              [[ "${lines["$parent_line_number,type"]}" != empty ]] &&
              [[ "${lines["$parent_line_number,length"]}" -lt "${lines["$line_number,length"]}" ]] ||
              [[ $parent_line_number -le "$first_content_line" ]]
        do
			((parent_line_number--))
		done
...
"
			...
			if [[ ! -z "${lines["$prev_line_number,list"]}" ]] && [[ "${lines["$prev_line_number,list"]}" != "${lines["$line_number,list"]}" ]]; then
				((parsed_segment++))
			...
			if [[ ! -z "${lines["$line_number,name"]}" ]]; then
			...
			if [[ ! -z "${parsed_text[$parsed_segment]}" ]]; then
				parsed_text[$parsed_segment]+="
"
			fi
			if [[ -z "${parsed_parent[$parsed_segment]}" && ! -z "${lines["$line_number,parentblock"]}" && "$parsed_segment" != "${lines["${lines["$line_number,parentblock"]},segment"]}" ]]; then
		...
		echo -e "$line_number.\t(${lines["$line_number,length"]})\t<p:${lines["$line_number,parentline"]}><b:${lines["$line_number,parentblock"]}><s:${lines["$line_number,segment"]}>\t[${lines["$line_number,type"]}]\n[${lines["$line_number,name"]}]\t${lines["$line_number,value"]}\n"
